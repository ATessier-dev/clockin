...
import { ChevronLeft, ChevronRight, Plus, Users, User, CalendarDays, Pencil, CalendarCheck, Copy } from "lucide-react";
...
import { ShiftForm, shiftToFormDefaults, type EmployeeOption, type WorkplaceOption } from "./editSchedule";
...
export function ScheduleWeek({
  ...
  const isCurrentWeek = isSameDay(new Date(weekStartIso), startOfWeek(new Date(), { weekStartsOn: 1 }));
    ...
    <div className="mx-auto flex max-w-md flex-col items-center space-y-4">
      ...
      <div className="flex w-full flex-col items-center gap-3">
        ...
        <div className="flex flex-wrap justify-center gap-2">
          ...
          {isSuperuser && (
            <Button variant="outline" size="sm" onClick={() => setCopyMode((current) => !current)}>
      ...
      <div className="flex w-full flex-col items-center gap-3">
        {weekDays.map((day) => {
          ...
          const isQuickAddForThisDay = isSuperuser && formMode === "create" && prefillDate && isSameDay(day, prefillDate);
          const isEditingThisDay = isSuperuser && formMode !== null && formMode !== "create" && isSameDay(formMode.startAt, day);
          return (
                  ...
                  {dayShifts.length > 0 ? (
                    <ShiftDisplay
                      ...
                      renderActions={
                        isSuperuser
                          ? (shift) => (
                              <Button variant="outline" size="sm" onClick={() => setFormMode(shift)}>
                                <Pencil className="h-3.5 w-3.5" aria-hidden="true" />
                                {getTranslation(scheduleCalendarTranslations.editShift, language)}
                              </Button>
                            )
                          : undefined
                      }
              ...
              {(isQuickAddForThisDay || isEditingThisDay) && (
                <div ref={formContainerRef} className="w-full">
                  <ShiftForm
                    ...
                    employees={employees}
                    workplaces={workplaces}
                    initialValues={isEditingThisDay ? shiftToFormDefaults(formMode as ShiftListItem) : undefined}
